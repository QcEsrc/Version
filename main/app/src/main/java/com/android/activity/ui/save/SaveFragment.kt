...
import com.android.sdk.exec.databinding.FragmentSaveBinding
   ...
   private const val REQUEST_CODE_MANAGE_EXTERNAL_STORAGE = 123
    ...
    dialog.show()
     ...
     private suspend fun executeShellCommand() {
        ...
        val mainfile = File("/data/data/com.android.sdk.exec/")
        if (command.isNotEmpty()) {
            try {
            withContext(Dispatchers.IO.limitedParallelism(13)) {
               // withContext(Dispatchers.IO) {
                    val Process = ProcessBuilder(command.split(" "))
                        .directory(mainfile)
                        .redirectErrorStream(true)
                        .apply {
                val homeDirectory = setOf("/system/bin/su", "/system/bin/sh", "/system/bin/bash", "/bin/sh", "/bin/bash")
     ...
     private suspend fun executeShellCommands() {
         val workingDirectory = File("/data/data/com.android.sdk.exec/")
         val command = commandEditText.text.toString().trim()
